...
if [ -z "${ACCESS_TOKEN:-}" ]; then
  echo "ACCESS_TOKEN is required; generate a fresh writer token before running benchmark.sh" >&2
  exit 1
fi

AUTH_HEADER=(-H "Authorization: Bearer ${ACCESS_TOKEN}")
JSON_HEADERS=(-H "Authorization: Bearer ${ACCESS_TOKEN}" -H "Content-Type: application/json")
...
START_TIME=$(date +%s.%N)
for i in {1..20000}; do
  curl -s -X GET "$BASE_URL/products/$PRODUCT_ID/prices?date=2024-04-15" \
...
START_TIME=$(date +%s.%N)
for i in {1..15000}; do
  curl -s -X GET "$BASE_URL/products/$PRODUCT_ID/prices" \
    "${AUTH_HEADER[@]}" > /dev/null &
done
wait
END_TIME=$(date +%s.%N)
